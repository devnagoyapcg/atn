    ...
    $(button_upload, {
        ...
        fileSelect : function(ev, options) {
            ...
            const opts = Object.assign({}, {
                onSelect : (files) => { chosenFile = files[0].name; },
                // onUpdate : (progress_pct) => { ... },
                // preview  : imageElement,
                field    : "file", //Field name (when using multiple, will add '[]' to the name)
                upload   : urlAtn + "upload" + "?orig=" + chosenFile,
                multiple : false
            }, options);
                let el = window._protected_reference = document.createElement("INPUT");
                el.name = opts.field;
                el.type = "file";
                el.accept = "file/*"; // this will accept csv file only; to accept different type of file, do "file/*" or "image/*"
